...
import { IMG_UTILS } from '../src/assets/imUtils';
...
function renderSquares(pieces: PieceRecord[]) {
  ...
  for (let row = 0; row < 8; row++) {
    for (let col = 0; col < 8; col++) {
      ...
      squares.push(
        <div key={`${row}-${col}`} css={squareStyles} style={{ backgroundColor: isDark ? 'lightgrey' : 'white' }}>
...
export function King() {
  return <Piece image={IMG_UTILS.king} alt="King" />;
}

export function Pawn() {
  return <Piece image={IMG_UTILS.pawn} alt="Pawn" />;
}
